...
public class JanusGraphId {
    ...
    /**
     * Converts a JanusGraph vertex id of a given vertex to the user provided numerical id as the inverse mapping of {@link #toVertexId(long)}.
     *
     * @param v Vertex
     * @return original user provided id, must be a number
     * @deprecated Use {@link org.janusgraph.graphdb.idmanagement.IDManager#fromVertexId(long)}
     */
    public static long fromVertexID(JanusGraphVertex v) {
        Preconditions.checkArgument(v.hasId() && v instanceof Number, "Invalid vertex provided: %s", v);
        return fromVertexId(((Number) v.id()).longValue());
    }
}
